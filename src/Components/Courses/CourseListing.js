...
export default function CourseListing(props) {
    ...
    const coursedata = useSelector(state => state.courselist);
    ...
    React.useEffect(()=>{
      if(searchdata){
        setcourselist(coursedata.filter(prod => {return prod.title.toLowerCase().includes(searchdata.toLowerCase())}));
          ...
          <Grid container spacing={3}>
                ...
                <Collapse in={open} timeout="auto" unmountOnExit>
                  ...
                  <ListItem >
                    ...
                    {props.catlist.map(catlist => (
                          ...
                          >
                            <Link className={classes.links} color='primary' underline='none' onClick={()=>{
                                setcourselist(coursedata.filter(course => course.category_id===catlist.category_id));
                                setTag(catlist.cat_name);
                                setOpen(!open);
                              }}>
                              {catlist.cat_name}
                            </Link>
            ...
            <List className={classes.root} >
                <li className={classes.listSection}>
                  ...
                  <ListItem >
                        <Link className={classes.links} color='primary' underline='none' onClick={()=>{setTag("All Categories");setcourselist(coursedata);}}>All Categories</Link>
                  </ListItem>
                  {props.catlist.map(cat => (
                        ...
                        >
                          <Link className={classes.innerlinks} onClick={()=>{setTag(cat.cat_name);setcourselist(coursedata.filter(course => course.category_id===cat.category_id));}} color='primary' underline='none'>
                            {cat.cat_name}
                          </Link>
                        </ListItem>
                    ))}
              ...
              <Paper component="form" onSubmit={(e)=>{e.preventDefault()}} className={classes.searchbar}>
                    <InputBase
                        ...
                        onChange={(e)=>{
                          ...
                          setcourselist(coursedata.filter(course => {return course.title.toLowerCase().includes(e.target.value.toLowerCase())}));}}
                    />
            ...
            <Grid item xs={12}>
              ...
              <DataView value={courselist} layout={'grid'} itemTemplate={itemTemplate} header={header} ></DataView>
